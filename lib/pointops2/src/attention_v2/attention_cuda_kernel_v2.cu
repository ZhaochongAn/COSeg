...
__global__ void attention_step1_forward_cuda_kernel_v2(
    int N, int M, int h, const unsigned int n_max, const float *q, const float *k,
...
template <unsigned int d>
__global__ void attention_step1_backward_cuda_kernel_v2(
    int N, int M, int h, const unsigned int n_max, const float *grad_out, const int *index0_offsets,
    const int *index1, const float *q, const float *k, float *grad_q, float *grad_k) {
...
void attention_step1_forward_cuda_launcher_v2(int N, int M, int h, int C, const unsigned int n_max,
    ...
    n_threads = n_threads == n_max ? n_threads : n_threads * 2;
  	...
  	switch (C / h) {
          case 16:
              attention_step1_forward_cuda_kernel_v2<16><<<blocks, n_threads, 0>>>(N, M, h, n_max, q, k, index0_offsets, index1, attn);
              break;
          case 32:
              attention_step1_forward_cuda_kernel_v2<32><<<blocks, n_threads, 0>>>(N, M, h, n_max, q, k, index0_offsets, index1, attn);
              break;
          case 64:
              attention_step1_forward_cuda_kernel_v2<64><<<blocks, n_threads, 0>>>(N, M, h, n_max, q, k, index0_offsets, index1, attn);
              break;
          case 80:
              attention_step1_forward_cuda_kernel_v2<80><<<blocks, n_threads, 0>>>(N, M, h, n_max, q, k, index0_offsets, index1, attn);
              break;
          default:
...
void attention_step1_backward_cuda_launcher_v2(int N, int M, int h, int C, const unsigned int n_max,
    ...
    n_threads = n_threads == n_max ? n_threads : n_threads * 2;
  	...
  	switch (C / h) {
          case 16:
              attention_step1_backward_cuda_kernel_v2<16><<<blocks, n_threads, 0>>>(N, M, h, n_max, grad_out, index0_offsets, index1, q, k, grad_q, grad_k);
              break;
          case 32:
              attention_step1_backward_cuda_kernel_v2<32><<<blocks, n_threads, 0>>>(N, M, h, n_max, grad_out, index0_offsets, index1, q, k, grad_q, grad_k);
              break;
          case 64:
              attention_step1_backward_cuda_kernel_v2<64><<<blocks, n_threads, 0>>>(N, M, h, n_max, grad_out, index0_offsets, index1, q, k, grad_q, grad_k);
              break;
          case 80:
              attention_step1_backward_cuda_kernel_v2<80><<<blocks, n_threads, 0>>>(N, M, h, n_max, grad_out, index0_offsets, index1, q, k, grad_q, grad_k);
              break;
          default:
              throw "d != 16 and d != 32 and d != 64 and d != 80";
    }
