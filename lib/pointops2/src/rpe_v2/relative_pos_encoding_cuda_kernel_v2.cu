...
__global__ void dot_prod_with_idx_forward_cuda_kernel_v2(
...
template <unsigned int d>
__global__ void dot_prod_with_idx_backward_cuda_kernel_v2(
    ...
    const int *rel_idx, const int *rel_idx_offsets, const int *sort_indices, float *output)
{
    ...
    switch (hdim) {
        case 16:
            dot_prod_with_idx_forward_cuda_kernel_v2<16><<<blocks, n_threads, 0>>>(N, M, h,
                q, index_q, k, index_k, table_q, table_k, rel_idx, rel_idx_offsets, sort_indices, output);
            break;
        case 32:
            dot_prod_with_idx_forward_cuda_kernel_v2<32><<<blocks, n_threads, 0>>>(N, M, h,
                q, index_q, k, index_k, table_q, table_k, rel_idx, rel_idx_offsets, sort_indices, output);
            break;
        case 64:
            dot_prod_with_idx_forward_cuda_kernel_v2<64><<<blocks, n_threads, 0>>>(N, M, h,
                q, index_q, k, index_k, table_q, table_k, rel_idx, rel_idx_offsets, sort_indices, output);
            break;
        case 80:
            dot_prod_with_idx_forward_cuda_kernel_v2<80><<<blocks, n_threads, 0>>>(N, M, h,
                q, index_q, k, index_k, table_q, table_k, rel_idx, rel_idx_offsets, sort_indices, output);
            break;
        default:
            throw "d != 16 and d != 32 and d != 64 and d != 80";
    }
}
    ...
    const int *sort_indices, float *grad_q, float *grad_k, float *grad_table_q, float *grad_table_k)
{
    ...
    switch (hdim) {
        case 16:
            dot_prod_with_idx_backward_cuda_kernel_v2<16><<<blocks, n_threads, 0>>>(N, M, h,
                grad_out, q, index_q, k, index_k, table_q, table_k, rel_idx, rel_idx_offsets,
                sort_indices, grad_q, grad_k, grad_table_q, grad_table_k);
            break;
        case 32:
            dot_prod_with_idx_backward_cuda_kernel_v2<32><<<blocks, n_threads, 0>>>(N, M, h,
                grad_out, q, index_q, k, index_k, table_q, table_k, rel_idx, rel_idx_offsets,
                sort_indices, grad_q, grad_k, grad_table_q, grad_table_k);
            break;
        case 64:
            dot_prod_with_idx_backward_cuda_kernel_v2<64><<<blocks, n_threads, 0>>>(N, M, h,
                grad_out, q, index_q, k, index_k, table_q, table_k, rel_idx, rel_idx_offsets,
                sort_indices, grad_q, grad_k, grad_table_q, grad_table_k);
            break;
        case 80:
            dot_prod_with_idx_backward_cuda_kernel_v2<80><<<blocks, n_threads, 0>>>(N, M, h,
                grad_out, q, index_q, k, index_k, table_q, table_k, rel_idx, rel_idx_offsets,
                sort_indices, grad_q, grad_k, grad_table_q, grad_table_k);
            break;
        default:
...
template <unsigned int d>
__global__ void dot_prod_with_idx_forward_cuda_kernel_v3(
    int N, int M, int h, int n_max, const float *q, const int *index_q_offsets,
    const float *k, const int *index_k, const float *table_q, const float *table_k,
    ...
    for(; n_idx < n_max; n_idx += blockDim.x){
        ...
        float sum = 0;
        for(int i = 0; i < d; i++){
            float table_q_scalar_i = table_q[r_idx1*C*3+h_idx*d*3+i*3] + table_q[r_idx2*C*3+h_idx*d*3+i*3+1]
                + table_q[r_idx3*C*3+h_idx*d*3+i*3+2];
            sum += query_vec[i] * table_q_scalar_i;
            float table_k_scalar_i = table_k[r_idx1*C*3+h_idx*d*3+i*3] + table_k[r_idx2*C*3+h_idx*d*3+i*3+1]
                + table_k[r_idx3*C*3+h_idx*d*3+i*3+2];
...
template <unsigned int d>
__global__ void dot_prod_with_idx_backward_cuda_kernel_v3(
    int N, int M, int h, int n_max, const float *grad_out, const float *q, const int *index_q_offsets,
    const float *k, const int *index_k, const float *table_q, const float *table_k,
    ...
    for(int tmp_n_idx = n_idx; tmp_n_idx < n_max; tmp_n_idx += blockDim.x){
        int m_idx = start + tmp_n_idx;
        if(m_idx < end){
            int k_idx = index_k[m_idx];
            int r_idx1 = rel_idx[m_idx*3], r_idx2 = rel_idx[m_idx*3+1], r_idx3 = rel_idx[m_idx*3+2];
...
void dot_prod_with_idx_forward_cuda_launcher_v3(int N, int M, int h, int hdim, int n_max,
    const float *q, const int *index_q_offsets, const float *k, const int *index_k,
    const float *table_q, const float *table_k, const int *rel_idx, float *output)
{
    ...
    switch (hdim) {
        case 16:
            dot_prod_with_idx_forward_cuda_kernel_v3<16><<<blocks, n_threads, 0>>>(N, M, h, n_max,
                q, index_q_offsets, k, index_k, table_q, table_k, rel_idx, output);
            break;
        case 32:
            dot_prod_with_idx_forward_cuda_kernel_v3<32><<<blocks, n_threads, 0>>>(N, M, h, n_max,
                q, index_q_offsets, k, index_k, table_q, table_k, rel_idx, output);
            break;
        case 64:
            dot_prod_with_idx_forward_cuda_kernel_v3<64><<<blocks, n_threads, 0>>>(N, M, h, n_max,
                q, index_q_offsets, k, index_k, table_q, table_k, rel_idx, output);
            break;
        case 80:
            dot_prod_with_idx_forward_cuda_kernel_v3<80><<<blocks, n_threads, 0>>>(N, M, h, n_max,
                q, index_q_offsets, k, index_k, table_q, table_k, rel_idx, output);
            break;
        default:
            throw "d != 16 and d != 32 and d != 64 and d != 80";
...
void dot_prod_with_idx_backward_cuda_launcher_v3(int N, int M, int h, int hdim, int n_max,
    const float *grad_out, const float *q, const int *index_q_offsets, const float *k,
    const int *index_k, const float *table_q, const float *table_k, const int *rel_idx,
    float *grad_q, float *grad_k, float *grad_table_q, float *grad_table_k)
{
    ...
    switch (hdim) {
        case 16:
            dot_prod_with_idx_backward_cuda_kernel_v3<16><<<blocks, n_threads, 0>>>(N, M, h, n_max,
                grad_out, q, index_q_offsets, k, index_k, table_q, table_k, rel_idx, grad_q, grad_k,
                grad_table_q, grad_table_k);
            break;
        case 32:
            dot_prod_with_idx_backward_cuda_kernel_v3<32><<<blocks, n_threads, 0>>>(N, M, h, n_max,
                grad_out, q, index_q_offsets, k, index_k, table_q, table_k, rel_idx, grad_q, grad_k,
                grad_table_q, grad_table_k);
            break;
        case 64:
            dot_prod_with_idx_backward_cuda_kernel_v3<64><<<blocks, n_threads, 0>>>(N, M, h, n_max,
                grad_out, q, index_q_offsets, k, index_k, table_q, table_k, rel_idx, grad_q, grad_k,
                grad_table_q, grad_table_k);
            break;
        case 80:
            dot_prod_with_idx_backward_cuda_kernel_v3<80><<<blocks, n_threads, 0>>>(N, M, h, n_max,
                grad_out, q, index_q_offsets, k, index_k, table_q, table_k, rel_idx, grad_q, grad_k,
                grad_table_q, grad_table_k);
            break;
        default:
...
template <unsigned int d>
__global__ void attention_step2_with_rel_pos_value_forward_cuda_kernel_v2(
    int N, int M, int h, int n_max, const float *attn, const float *v,
    const int *index0_offsets, const int *index1, const float *table,
    const int *rel_idx, float *output) {
...
template <unsigned int d>
__global__ void attention_step2_with_rel_pos_value_backward_cuda_kernel_v2(
    int N, int M, int h, int n_max, const float *grad_out, const int *index0_offsets,
    const int *index1, const float *attn, const float *v, const float *table,
...
void attention_step2_with_rel_pos_value_forward_cuda_launcher_v2(int N, int M, int h, int hdim,
    int n_max, const float *attn, const float *v, const int *index0_offsets,
    ...
    switch (hdim) {
        case 16:
            attention_step2_with_rel_pos_value_forward_cuda_kernel_v2<16><<<blocks, n_threads, 0>>>(
                N, M, h, n_max, attn, v, index0_offsets, index1, table, rel_idx, output);
            break;
        case 32:
            attention_step2_with_rel_pos_value_forward_cuda_kernel_v2<32><<<blocks, n_threads, 0>>>(
                N, M, h, n_max, attn, v, index0_offsets, index1, table, rel_idx, output);
            break;
        case 64:
            attention_step2_with_rel_pos_value_forward_cuda_kernel_v2<64><<<blocks, n_threads, 0>>>(
                N, M, h, n_max, attn, v, index0_offsets, index1, table, rel_idx, output);
            break;
        case 80:
            attention_step2_with_rel_pos_value_forward_cuda_kernel_v2<80><<<blocks, n_threads, 0>>>(
                N, M, h, n_max, attn, v, index0_offsets, index1, table, rel_idx, output);
            break;
        default:
            throw "d != 16 and d != 32 and d != 64 and d != 80";
...
void attention_step2_with_rel_pos_value_backward_cuda_launcher_v2(int N, int M, int h, int hdim,
    int n_max, const float *grad_out, const int *index0_offsets, const int *index1, const float *attn,
    const float *v, const float *table, const int *rel_idx, float *grad_attn, float *grad_v, float *grad_table) {
    ...
    switch (hdim) {
        case 16:
            attention_step2_with_rel_pos_value_backward_cuda_kernel_v2<16><<<blocks, n_threads, 0>>>(
                N, M, h, n_max, grad_out, index0_offsets, index1, attn, v, table, rel_idx, grad_attn, grad_v, grad_table);
            break;
        case 32:
            attention_step2_with_rel_pos_value_backward_cuda_kernel_v2<32><<<blocks, n_threads, 0>>>(
                N, M, h, n_max, grad_out, index0_offsets, index1, attn, v, table, rel_idx, grad_attn, grad_v, grad_table);
            break;
        case 64:
            attention_step2_with_rel_pos_value_backward_cuda_kernel_v2<64><<<blocks, n_threads, 0>>>(
                N, M, h, n_max, grad_out, index0_offsets, index1, attn, v, table, rel_idx, grad_attn, grad_v, grad_table);
            break;
        case 80:
            attention_step2_with_rel_pos_value_backward_cuda_kernel_v2<80><<<blocks, n_threads, 0>>>(
                N, M, h, n_max, grad_out, index0_offsets, index1, attn, v, table, rel_idx, grad_attn, grad_v, grad_table);
            break;
        default:
            throw "d != 16 and d != 32 and d != 64 and d != 80";
