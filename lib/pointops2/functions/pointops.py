...
class FurthestSampling(Function):
    ...
    def forward(ctx, xyz, offset, new_offset):
        ...
        output: idx: (m)
        """
        assert xyz.is_contiguous() and offset.is_contiguous() and new_offset.is_contiguous()
...
class KNNQuery(Function):
    ...
    def forward(ctx, nsample, xyz, new_xyz, offset, new_offset):
        ...
        assert offset.is_contiguous() and new_offset.is_contiguous()
        m = new_xyz.shape[0]
...
class BallQuery(Function):
    ...
    def forward(ctx, radius, nsample, xyz, new_xyz, offset, new_offset):
        ...
        idx = torch.cuda.IntTensor(m, nsample).zero_()
        dist2 = torch.cuda.FloatTensor(m, nsample).zero_()
        pointops_cuda.ball_query_cuda(m, radius, nsample, xyz, new_xyz, offset, new_offset, idx, dist2)
        return idx, torch.sqrt(dist2)
...
class AttentionStep1(Function):
    ...
    def forward(ctx, q, k, index0, index1):
        ...
        pointops_cuda.attention_step1_forward_cuda(N_q, M, h, C, q, k, index0, index1, output)
        ctx.N_q = N_q
        ctx.N_k = N_k
        ctx.C = C
    ...
    def backward(ctx, grad_output):
        ...
        assert q.is_contiguous() and k.is_contiguous() and grad_output.is_contiguous()
        assert index0.is_contiguous() and index1.is_contiguous()
...
class AttentionStep1_v2(Function):
    ...
    def forward(ctx, q, k, index1, index0_offsets, n_max):
        ...
        output = torch.cuda.FloatTensor(M, h).zero_()
        pointops_cuda.attention_step1_forward_cuda_v2(N_q, M, h, C, n_max, q, k, index0_offsets, index1, output)
        ctx.N_q = N_q
        ctx.N_k = N_k
        ctx.C = C
        ctx.n_max = n_max
    ...
    def backward(ctx, grad_output):
        ...
        grad_output = grad_output.contiguous()
        assert q.is_contiguous() and k.is_contiguous() and grad_output.is_contiguous()
        assert index0_offsets.is_contiguous() and index1.is_contiguous()
...
class AttentionStep2(Function):
    ...
    def forward(ctx, attn, v, index0, index1):
        ...
        output = torch.cuda.FloatTensor(N_q, h, C_div_h).zero_()
    ...
    def backward(ctx, grad_output):
        ...
        grad_output = grad_output.contiguous()
        assert attn.is_contiguous() and v.is_contiguous() and grad_output.is_contiguous()
        assert index0.is_contiguous() and index1.is_contiguous()

        grad_attn = torch.cuda.FloatTensor(M, h).zero_()
        grad_v = torch.cuda.FloatTensor(N_v, h, C_div_h).zero_()
...
class DotProdWithIdx(Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_output = grad_output.contiguous()
        assert q.is_contiguous() and index.is_contiguous() and table.is_contiguous()
        assert rel_idx.is_contiguous() and grad_output.is_contiguous()
...
class DotProdWithIdx_v2(Function):
    ...
    def forward(ctx, q, index_q, k, index_k, table_q, table_k, rel_idx):
        ...
        output: output: [M, h]
        """
        assert q.is_contiguous() and index_q.is_contiguous() and table_q.is_contiguous() and rel_idx.is_contiguous()
        assert k.is_contiguous() and index_k.is_contiguous() and table_k.is_contiguous()
        ...
        output = torch.cuda.FloatTensor(M, h).zero_()
        pointops_cuda.dot_prod_with_idx_forward_cuda_v2(N, M, h, hdim, n_max, T, q, index_q, k, index_k, table_q,
            table_k, rel_idx, rel_idx_offsets.int(), sort_indices.int(), output)
    ...
    def backward(ctx, grad_output):
        ...
        grad_output = grad_output.contiguous()
        assert q.is_contiguous() and index_q.is_contiguous() and table_q.is_contiguous()
        assert k.is_contiguous() and index_k.is_contiguous() and table_k.is_contiguous()
        assert rel_idx.is_contiguous() and rel_idx_offsets.is_contiguous() and sort_indices.is_contiguous() and grad_output.is_contiguous()
        ...
        pointops_cuda.dot_prod_with_idx_backward_cuda_v2(N_q, M, h, hdim, n_max, T, grad_output, q, index_q, k, index_k,
            table_q, table_k, rel_idx, rel_idx_offsets.int(), sort_indices.int(), grad_q, grad_k, grad_table_q, grad_table_k)
...
class DotProdWithIdx_v3(Function):
    ...
    def forward(ctx, q, index_q_offsets, n_max, k, index_k, table_q, table_k, rel_idx):
        ...
        output: output: [M, h]
        """
        assert q.is_contiguous() and index_q_offsets.is_contiguous() and table_q.is_contiguous()
        assert k.is_contiguous() and index_k.is_contiguous() and table_k.is_contiguous() and rel_idx.is_contiguous()
    ...
    def backward(ctx, grad_output):
        ...
        grad_output = grad_output.contiguous()
        assert q.is_contiguous() and index_q_offsets.is_contiguous() and table_q.is_contiguous() and rel_idx.is_contiguous()
        assert k.is_contiguous() and index_k.is_contiguous() and table_k.is_contiguous() and grad_output.is_contiguous()
        ...
        pointops_cuda.dot_prod_with_idx_backward_cuda_v3(N_q, M, h, hdim, n_max, grad_output, q, index_q_offsets,
            k, index_k, table_q, table_k, rel_idx, grad_q, grad_k, grad_table_q, grad_table_k)
...
class AttentionStep2WithRelPosValue(Function):
    ...
    def forward(ctx, attn, v, index0, index1, table, rel_idx):
        ...
        """
        assert attn.is_contiguous() and v.is_contiguous() and table.is_contiguous()
        assert index0.is_contiguous() and index1.is_contiguous() and rel_idx.is_contiguous()
    ...
    def backward(ctx, grad_output):
        ...
        grad_output = grad_output.contiguous()
        assert attn.is_contiguous() and v.is_contiguous() and grad_output.is_contiguous() and table.is_contiguous()
        assert index0.is_contiguous() and index1.is_contiguous() and rel_idx.is_contiguous()
        ...
        pointops_cuda.attention_step2_with_rel_pos_value_backward_cuda(N_q, M, h, hdim, grad_output,
            index0, index1, attn, v, table, rel_idx, grad_attn, grad_v, grad_table)
...
class AttentionStep2WithRelPosValue_v2(Function):
    ...
    def forward(ctx, attn, v, index0_offsets, n_max, index1, table, rel_idx):
        ...
        output: output: [N, h, hdim]
        """
        assert attn.is_contiguous() and v.is_contiguous() and index0_offsets.is_contiguous()
        assert index1.is_contiguous() and table.is_contiguous() and rel_idx.is_contiguous()

        M, h = attn.shape
        N_v, h, hdim = v.shape
        N_q = index0_offsets.shape[0] - 1

        output = torch.cuda.FloatTensor(N_q, h, hdim).zero_()
        pointops_cuda.attention_step2_with_rel_pos_value_forward_cuda_v2(N_q, M, h, hdim, n_max, attn, v, index0_offsets, index1, table, rel_idx, output)
    ...
    def backward(ctx, grad_output):
        ...
        assert grad_output.is_contiguous() and table.is_contiguous() and rel_idx.is_contiguous()

        grad_attn = torch.cuda.FloatTensor(M, h).zero_()
        grad_v = torch.cuda.FloatTensor(N_v, h, hdim).zero_()
        grad_table = torch.cuda.FloatTensor(L, h, hdim, 3).zero_()

        pointops_cuda.attention_step2_with_rel_pos_value_backward_cuda_v2(N_q, M, h, hdim, n_max, grad_output,
            index0_offsets, index1, attn, v, table, rel_idx, grad_attn, grad_v, grad_table)
...
class Subtraction(Function):
    ...
    def forward(ctx, input1, input2, idx):
        ...
        output:  (n, nsample, c)
        """
        assert input1.is_contiguous() and input2.is_contiguous() and idx.is_contiguous()
...
class Aggregation(Function):
    ...
    def forward(ctx, input, position, weight, idx):
        ...
        output: (n, c)
        """
        assert input.is_contiguous() and position.is_contiguous() and weight.is_contiguous() and idx.is_contiguous()
    ...
    def backward(ctx, grad_output):
        ...
        pointops_cuda.aggregation_backward_cuda(n, nsample, c, w_c, input, position, weight, idx,
            grad_output, grad_input, grad_position, grad_weight)
