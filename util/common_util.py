...
def evaluate_metric(
    pred_label, gt_label, label2class, test_classes, ignore_index=255
):
    """_summary_
    ...
    intersec_classes = torch.tensor(
        [0 for _ in range(NUM_CLASS)], device="cuda"
    )
...
def load_pretrain_checkpoint(model, pretrain_checkpoint_path, gpu):
    ...
    if pretrain_checkpoint_path is not None:
        ...
        pretrained_dict = torch.load(
            os.path.join(pretrain_checkpoint_path, "model", "model_best.pth"),
...
def memory_use():
    BYTES_IN_GB = 1024**3
    return (
        "ALLOCATED: {:>6.3f} ({:>6.3f})  CACHED: {:>6.3f} ({:>6.3f})".format(
