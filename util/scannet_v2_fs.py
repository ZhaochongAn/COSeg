...
class Scannetv2_base(Dataset):
    def __init__(
        ...
        self.class_count = 21
        # Classes: {0:'unannotated', 1:'wall', 2:'floor', 3:'chair',
        #   4:'table', 5:'desk', 6:'bed', 7:'bookshelf', 8:'sofa', 9:'sink',
        #   10:'bathtub', 11:'toilet', 12:'curtain', 13:'counter', 14:'door',
        #   15:'window', 16:'shower curtain', 17:'refridgerator', 18:'picture',
        #   19:'cabinet', 20:'otherfurniture'}
        class_names = open(
            os.path.join(
        ...
        if cvfold == 0:
            ...
            raise NotImplementedError(
                f"Unknown cvfold ({cvfold}). [Options: 0,1]"
            )

        all_classes = [i for i in range(1, self.class_count)]
        self.train_classes = [
...
class Scannetv2(Scannetv2_base):
    """
    The Scannetv2 dataset class used for backbone pretraining.
    """
...
class Scannetv2_FS(Scannetv2_base):
    """
    The Scannetv2 dataset class used for few-shot learning.
    """
    ...
    def __init__(
        ...
        if split == "train":
            ...
            raise NotImplementedError(
                f"Unkown mode {split}! [Options: train/test]"
            )
...
class Scannetv2_FS_TEST(Dataset):
    def __init__(
        self,
        split="val",
        data_root="trainval",
        ...
        if split == "val":
            self.test_data_path = os.path.join(
                ...
                "Scan_%d_N_%d_K_%d_episodes_%d_pts_%d_vs_%.2f_rdmsp"
                % (
                    cvfold,
                    n_way,
                    k_shot,
            ...
            self.test_data_path = os.path.join(
                os.path.dirname(data_root),
                "Scan_%d_N_%d_K_%d_test_episodes_%d_pts_%d_vs_%.2f"
                % (
                    cvfold,
                    n_way,
                    k_shot,
