...
def collect_point_label(anno_path, out_filename, file_format="numpy"):
    ...
    if file_format == "txt":
        ...
        print(
            "ERROR!! Unknown file format: %s, please use txt or numpy."
            % (file_format)
        )
        exit()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_path",
        default="datasets/S3DIS/Stanford3dDataset_v1.2_Aligned_Version",
    ...
    CLASS_NAMES = [
        ...
        for x in open(
            os.path.join(
                ROOT_DIR, "datasets/S3DIS/meta", "s3dis_classnames.txt"
            )
        )
    ]
    ...
    for folder in folders:
        ...
        for scene_path in scene_paths:
            ...
            try:
                collect_point_label(
                    anno_path, os.path.join(SAVE_PATH, out_filename)
                )
            except:
