...
def collect_point_label(scene_path, scene_name, out_filename):
    ...
    mesh_seg_filename = os.path.join(
        scene_path, "%s_vh_clean_2.0.010000.segs.json" % (scene_name)
    )
    ...
    annotation_filename = os.path.join(
        scene_path, "%s.aggregation.json" % (scene_name)
    )
    ...
    for i in range(len(instance_segids)):
        ...
        semantic_labels_list.append(
            np.ones((instance_points.shape[0], 1)) * label
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_path",
        default="datasets/ScanNet/scans",
        help="Directory to dataset",
    )
    parser.add_argument(
        "--save_path",
        default="datasets/ScanNet",
    ...
    SAVE_PATH = os.path.join(args.save_path, "scenes", "data")
    if not os.path.exists(SAVE_PATH):
        os.makedirs(SAVE_PATH)

    meta_path = os.path.join(
        os.path.join(ROOT_DIR, "datasets/ScanNet"), "meta"
    )
    CLASS_NAMES = [
        x.rstrip()
        for x in open(os.path.join(meta_path, "scannet_classnames.txt"))
    ]
    label_mapping_file = os.path.join(
        meta_path, "scannetv2-labels.combined.tsv"
    )
    RAW2SCANNET = get_raw2scannet_label_map(label_mapping_file)

    scene_paths = [
        os.path.join(DATA_PATH, o)
    ...
    for scene_path in scene_paths:
        ...
        try:
            ...
            collect_point_label(
                scene_path, scene_name, os.path.join(SAVE_PATH, out_filename)
            )
        except:
